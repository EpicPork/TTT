...
from TTT_pkg.database import insert_game_result, update_leaderboard
...
class GameBoard:
    ...
    # Check for a winning condition (rows, columns, diagonals)
        ...
        if self.board[self.size - 1][0] != " " and all(self.board[self.size - 1 - i][i] == self.board[self.size - 1][0] for i in range(1, self.size)):
